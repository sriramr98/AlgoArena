...
module.exports = {
  ...
  prepare(userCode, problem, testCase) {
    ...
    Object.keys(problem.input).forEach((key) => {
      ...
      if (inputType === 'array' || inputType === 'object') {
        variableDeclarations.push(
          `let ${key} = ${JSON.stringify(testCase.input[key])};`,
        );
    ...
    codeToExecute = codeToExecute.replace('{function_call}', functionCall);

    let printResult = '';
    if (problem.executionMode === 'return') {
      printResult = `console.log(result);`;
    } else {
      const outputVariable = Object.keys(problem.input).filter(
        (key) => problem.input[key].output,
      )[0];
      printResult = `console.log(${outputVariable});`;
    }

    codeToExecute = codeToExecute.replace('{print_result}', printResult);
